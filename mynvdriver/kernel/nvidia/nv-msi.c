...
void NV_API_CALL nv_init_msix(nv_state_t *nv)
{
    ...
    num_intr = nv_get_max_irq(nvl->dev);

    if (num_intr > NV_RM_MAX_MSIX_LINES)
    {
        nv_printf(NV_DBG_INFO, "NVRM: Reducing MSI-X count from %d to the "
    ...
    {
        nv_printf(NV_DBG_ERRORS, "NVRM: Failed to allocate MSI-X entries.\n");
        return;
    }
...
failed:
    ...
    nv_printf(NV_DBG_ERRORS, "NVRM: Failed to enable MSI-X.\n");
}
