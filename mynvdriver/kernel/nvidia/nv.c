...
void nv_check_pci_config_space(nv_state_t *nv, BOOL check_the_bars)
{
    ...
    if ((nvl->dev == NULL) || NV_IS_DEVICE_IN_SURPRISE_REMOVAL(nv))
    {
        nv_printf(NV_DBG_INFO,
            "NVRM: GPU is lost, skipping PCI config space check\n");
        return;
    }
...
static int nv_open_device(nv_state_t *nv, nvidia_stack_t *sp)
{
    ...
    status = nv_check_pci_config_space_and_gpu_state(sp, nv, TRUE,
                                                     TRUE, NV_MAY_SLEEP());
    if (status == NV_ERR_GPU_IS_LOST)
    {
        nv_printf(NV_DBG_INFO, "NVRM: Device in removal process\n");
        return -ENODEV;
    }
...
)
{
    ...
    /* Intentionally not checking the status here */
    (void) nv_check_pci_config_space_and_gpu_state(sp, nv, TRUE,
                                                   TRUE, NV_MAY_SLEEP());
...
)
{
    ...
    if (status == NV_ERR_GPU_IS_LOST)
    {
        nv_printf(NV_DBG_INFO, "NVRM: GPU is lost, skipping nvidia_poll\n");
        return POLLHUP;
    }
...
)
{
    ...
    {
        status = nv_check_pci_config_space_and_gpu_state(sp, nv, FALSE,
                                                         TRUE, NV_MAY_SLEEP());
        if (status == NV_ERR_GPU_IS_LOST)
        {
            nv_printf(NV_DBG_INFO,
                "NVRM: GPU is lost, skipping nvidia_probe\n");
            goto err_not_supported;
        }
...
)
{
    ...
    if (status == NV_ERR_GPU_IS_LOST)
    {
        nv_printf(NV_DBG_INFO, "NVRM: GPU is lost, skipping PM event\n");
        goto failure;
    }
