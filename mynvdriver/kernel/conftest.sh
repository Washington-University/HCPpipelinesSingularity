...
compile_test() {
    case "$1" in
      ...
      pci_dma_mapping_error)
        ...
        compound_order)
            #
            # Determine if compound_order() and compound_head() are present.
            #
            # These compound page utilities were added by:
            #   2007-05-06  d85f33855c303acfa87fa457157cef755b6087df
            #
            CODE="
            #include <linux/mm.h>
            void conftest_compound_order() {
              compound_order();
            }"

            compile_check_conftest "$CODE" "NV_COMPOUND_ORDER_PRESENT" "" "functions"
        ;;
        dma_buf_owner)
            #
