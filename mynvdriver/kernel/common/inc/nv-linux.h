 ...
 */
static inline NV_STATUS nv_check_pci_config_space_and_gpu_state(
                            nvidia_stack_t *sp, nv_state_t *nv, NvBool checkBar,
                            NvBool acquireLocks, NvBool mayBlock)
{
    return NV_OK;
}
#else
 ...
 * functionality.
 * Also check if GPU is present on the bus by checking flag
                            ...
                            NvBool acquireLocks, NvBool mayBlock)
{
        ...
        (((nv)->flags & NV_FLAG_CONTROL) == 0))
    {
        if ((nv)->flags & NV_FLAG_USE_BAR0_CFG)
            rm_check_pci_config_space(sp, nv, checkBar, acquireLocks, mayBlock);
        else
            nv_check_pci_config_space(nv, checkBar);
    }
